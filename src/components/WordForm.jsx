...
export default function WordForm({earthWord, alienWord , category , id , flag, editSetter}) {
    const {words, wordsDispatcher} = useContext(WordsContext)
    const [inputs,setInputs]=useState({earthWord: earthWord, alienWord:alienWord, category:category, id :id
    })
    ...
    const handleSubmit = (e) => {
        ...
        if (inputs.earthWord && inputs.alienWord && inputs.category) {
            ...
            fetch("http://localhost:7070/words", {method: 'POST',     headers: {
            ...
            if(flag){
                editSetter({id: 0, earthWord: '', alienWord: '', category: '', flag: false});
            }
    ...
    return(
        <div className="word-input-container text-black-100">
            ...
            <h1>{flag ? 'Update a word!': 'Enter a new translated word!!🔁'}</h1>
                <div className="mb-4">
                    <label htmlFor="earthWord" className="block text-gray-700 font-semibold mb-2">Human Word:</label>
                    <input
                        type="text"
                        id="earthWord"
                        name="earthWord"
                ...
                <div className="mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <button className="text-md text-gray-500 h-6 w-6 flex items-center justify-center hover:bg-gray-100 rounded-full focus:outline-none" onClick={getAndSetRandomName}>
                        🎲
                        </button>
                    </div>
                ...
                <div className="text-center">
                    <button type="submit" className={`w-full py-2 px-4 bg-green-500 text-white font-semibold rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-ring-500 ${inputs.category ? '' : 'opacity-50 cursor-not-allowed'}`}>
